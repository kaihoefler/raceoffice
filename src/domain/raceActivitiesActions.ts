 ...
 *
 * Rules:
 * - pointsSprint / elimination: filter `data.results` by bib
 * - DNS / DSQ: drop the whole activity if its bib is no longer allowed
 * - empty points/elimination activities are removed completely
 */
...
export function removeBibFromActivities(activities: RaceActivity[], bib: number): RaceActivity[] {
  ...
  for (const activity of Array.isArray(activities) ? activities : []) {
    if (activity.type === "pointsSprint" || activity.type === "elimination") {
